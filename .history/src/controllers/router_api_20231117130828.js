...
module.exports.get_list = async (req, res, next) => {
  ...
  try {
    ...
    const result = await router_api.findAll({
      ...
      attributes: ['id', 'group_id', 'name', 'path', 'notes', 'status', 'createdAt', 'updatedAt'],
      order: [
...
module.exports.do_edit = async (req, res, next) => {
  const { router_api } = require('../models/routerApi')
  try {
    const { id, group_id, name, path, notes, status } = req.body;
    const data = { group_id: group_id, name: name, path: path, notes: notes, status: status, logging: console.log };
    if (id) {
      const where = { id: id }
