...
module.exports.get_list_tree = async (req, res, next) => {
  const { router, Op } = require('../models/routerv');
...
module.exports.get_list = async (req, res, next) => {
  ...
  try {
    ...
    const result = await routerv.findAll({
      where: {
        [Op.or]: [
          { name: { [Op.like]: `%${name}%` } },
          { title: { [Op.like]: `%${title}%` } },
...
module.exports.do_edit = async (req, res, next) => {
  const { routerv } = require('../models/routerv')
  try {
    ...
    if (id) {
      const where = { id: id }
      const result = await routerv.update(data, { where: where });
      res.send({
        status: 200,
        message: 'success',
        data: result
      ...
      const result = await routerv.create(data);
      res.send({
        status: 200,
        message: 'success',
        data: result
