...
module.exports.do_authorize_api = async (req, res, next) => {
  ...
  try {
    ...
    // 计算需要创建和删除的记录
    const recordsToCreate = transformedArray.filter(item =>
      !existingRouterApiPairs.some(pair =>
        pair.router_id === item.router_id && pair.api_id === item.api_id
      )
    ); //通过比较 dataArray 中的每一项的 router_id 和 api_id 组合是否在 existingRouterApiPairs 中存在，筛选出在数据库中不存在的记录，即需要插入的记录。
    ...
    ); //通过比较数据库中已存在的记录与 dataArray 中的每一项的 router_id 和 api_id 组合，筛选出在 dataArray 中不存在的记录，即需要从数据库中删除的记录。
