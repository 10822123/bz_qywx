  ...
  try {
    ...
    const recordsToDelete = existingRecords.filter(record =>
      !bind_array.some(item => {
        const condition = item.router_id === record.get('router_id') && item.api_id === record.get('api_id');
        console.log(`Comparing ${item.router_id}, ${item.api_id} with ${record.get('router_id')}, ${record.get('api_id')}. Result: ${condition}`);
        return condition;
      })
    );
    ...
    console.log('bind_array types:', bind_array.map(item => typeof item.router_id));
    console.log('existingRecords types:', existingRecords.map(record => typeof record.get('router_id')));
