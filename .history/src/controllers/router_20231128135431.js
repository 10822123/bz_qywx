...
module.exports.list = async (req, res, next) => {
  ...
  function convertToTree(data) {
    ...
    function createNode(item) {
        nodeMap[item.id] = { id: item.id, parent_id: item.parent_id, name: item.name, meta: item.meta, children: [] };
    }
  
    function addChildToParent(item) {
        const node = nodeMap[item.id];
  
        if (!node) {
            // 如果当前节点不存在，可能是由于数据顺序问题，可以在这里处理或记录错误。
        ...
        if (item.parent_id === 0) {
            ...
            if (!parent) {
                // 如果父节点不存在，可能是由于数据顺序问题，可以在这里处理或记录错误。
