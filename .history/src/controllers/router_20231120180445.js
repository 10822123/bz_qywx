...
module.exports.get_list = async (req, res, next) => {
  ...
  try {
    ...
    const { count:total, rows: result } = await router.findAndCountAll ({
      attributes: ['id', 'parent_id', 'api_id', 'name', 'title', 'path', 'component', 'hidden', 'levelHidden', 'icon', 'no_keepalive', 'badge', 'status', 'createdAt', 'updatedAt'],
      include: [
        {
          model: router_api,
          attributes: ['id', 'group_id', 'name', 'path', 'types', 'notes', 'status', 'deletedAt', 'createdAt', 'updatedAt'],
         through: router_bind_api,
        },
      ],
      where: literal(`"router"."deletedAt" IS NULL AND "router_apis->router_bind_api"."deletedAt" IS NULL`),
      where: {
        [Op.or]: [
          { name: { [Op.like]: `%${name}%` } },
          { title: { [Op.like]: `%${title}%` } },
