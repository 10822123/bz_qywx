...
module.exports.get_list = async (req, res, next) => {
  ...
  const { routerbind__api } = require('../models/routerbind__api');
  try {
    ...
    // 建立模型之间的关联关系
    router.belongsToMany(router_api, { through: routerbind__api, foreignKey: 'router_id' });
    router.belongsToMany(router, { through: routerbind__api, foreignKey: 'api_id' });
