...
module.exports.get_list = async (req, res, next) => {
  ...
  const { router_bind__api } = require('../models/router_bind__api');
  try {
    ...
    // 建立模型之间的关联关系
router.belongsToMany(router_api, { through: router_bind__api, foreignKey: 'router_id' });
router_api.belongsToMany(router, { through: router_bind__api, foreignKey: 'api_id' });
