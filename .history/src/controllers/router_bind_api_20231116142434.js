...
module.exports.do_authorize_api = async (req, res, next) => {
  ...
  try {
    ...
    const recordsToCreate = dataArray.filter(item => !existingRouterApiPairs.some(pair => pair.router_id === item.router_id && pair.api_id === item.api_id));  //通过比较 dataArray 中的每一项的 router_id 和 api_id 组合是否在 existingRouterApiPairs 中存在，筛选出在数据库中不存在的记录，即需要插入的记录。
    const recordsToDelete = existingRecords.filter(record => !dataArray.some(item => item.router_id === record.router_id && item.api_id === record.api_id));  //通过比较数据库中已存在的记录与 dataArray 中的每一项的 router_id 和 api_id 组合，筛选出在 dataArray 中不存在的记录，即需要从数据库中删除的记录。
    


    if (id) {
      const where = { id: id }
      const result = await router_bind_api.update(data, { where: where });
      res.send({
        status: 200,
        message: 'success',
        data: result
      ...
      const result = await router_bind_api.create(data);
      res.send({
        status: 200,
        message: 'success',
        data: result
